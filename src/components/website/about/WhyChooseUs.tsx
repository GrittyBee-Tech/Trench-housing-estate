  ...
  return (
    <section className='bg-[#f5f5f5] py-10 px-7 sm:px-12 xl:px-24 grid grid-cols-4 grid-rows-2 gap-5'>
      <div className='flex flex-col justify-center col-span-2 row-span-2'>
        <h2 className='text-primary text-4xl font-Montserrat font-semibold'>
          Why Choose Us
        </h2>
        <p className='font-Montserrat text-[#515151] mt-4'>
          Our mission is to provide exceptional real estate services with
          integrity, transparency, and a commitment to excellence. We strive to
          create lasting relationships with our clients, ensuring that they have
      ...
      {reasons.map(({ icon, title, text }) => (
        <article
          key={title}
          className='bg-white p-5 rounded-md col-span-1 row-span-1 flex flex-col gap-2.5'
        >
          <span className='p-2 rounded-full w-fit bg-[#F5F5F5] shadow-sm'>
            <Icon color='#001A35' size={22} type={icon} />
          </span>
          <h5 className='text-[#001A35] text-xl font-semibold font-Montserrat'>
            {title}
          </h5>
